...
# Imported for its side effect: loading rdkit registers the C++ Mol classes so
# pandas DataFrames pickled with RDKit Mol objects can be unpickled here.
...
def save_molecular_dataframe(df, filename, chapter="ch01", compress=True):
    ...
    """
    # Create the artifacts directory if it doesn't exist. Anchor to the repo
    # root (the directory containing utils.py) so saving works regardless of the
    # kernel's CWD, matching load_molecular_dataframe below.
    repo_root = Path(__file__).resolve().parent
    save_dir = repo_root / "artifacts" / chapter
...
def list_saved_dataframes(chapter="ch01"):
    ...
    """
    # Anchor to the repo root so listing is independent of the kernel's CWD,
    # matching save/load_molecular_dataframe.
    repo_root = Path(__file__).resolve().parent
    save_dir = repo_root / "artifacts" / chapter
    ...
    for file_path in saved_files:
        ...
        file_info.append(
            {"filename": file_path.name, "size_mb": file_size_mb, "modified": modified_time}
        )
